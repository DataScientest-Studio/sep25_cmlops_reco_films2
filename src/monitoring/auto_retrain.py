...
def main():
    ...
    try:
        ...
        # Drift sur les films
        print("\n   Analyse drift FILMS:")
        common_movie_cols = list(set(movie_matrix_ref.columns) & set(movie_matrix_current.columns))
        ...
        common_user_cols = list(set(user_matrix_ref.columns) & set(user_matrix_current.columns))
        ...
        decision, reason = should_retrain(movie_drift_score, user_drift_score, threshold=0.3)
        ...
        print(f"   Drift films: {movie_drift_score:.2%}")
        print(f"   Drift utilisateurs: {user_drift_score:.2%}")
        print(f"   Réentraînement: {' EFFECTUÉ' if retrain_success else '❌ NON NÉCESSAIRE' if not decision else '⚠️ ÉCHOUÉ'}")
